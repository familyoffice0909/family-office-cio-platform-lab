...
function foAutomationLog_(level, module, action, message) {
  const sheet = foEnsureSheet_(
    foDashboard_(),
    foAutomationConfig_().AUTOMATION_LOG,
    [
  ...
  sheet.appendRow([
    ...
    foAutomationConfig_().AUTOMATION_VERSION,
    foAutomationConfig_().BASELINE,
    foGetActiveUser_()
  ]);
}
...
function foGetAutomationVersion() {
  return {
    ...
    automationVersion: foAutomationConfig_().AUTOMATION_VERSION,
    baseline: foAutomationConfig_().BASELINE,
    release: foAutomationConfig_().RELEASE,
...
function foAutomationBootstrap() {
  ...
  try {
    ...
    foEnsureSheet_(foDashboard_(), foAutomationConfig_().AUTOMATION_LOG, [
      'Timestamp', 'Level', 'Module', 'Action', 'Message',
      'Platform Version', 'Automation Version', 'Baseline', 'User'
    ]);

    foEnsureSheet_(foDashboard_(), foAutomationConfig_().HEALTH_SHEET, [
